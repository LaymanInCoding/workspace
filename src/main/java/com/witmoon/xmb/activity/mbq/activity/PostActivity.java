...
public class PostActivity extends BaseActivity {
    ...
    protected void  initialize(Bundle savedInstanceState) {
        ...
        add.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent intent = new Intent(PostActivity.this, LocalAlbum.class);
                startActivityForResult(intent, ImageUtils.REQUEST_CODE_GETIMAGE_BYCROP);
            }
        });
        IntentFilter finishActivity = new IntentFilter(Const.INTENT_ACTION_REFRESH_CIRCLE);
        ...
        switch (view.getId()) {
            case R.id.toolbar_right_text:
                ...
                if(StringUtils.isEmpty(title)){
                    XmbUtils.showMessage(PostActivity.this,"请输入标题~");
                    return;
                } else if (StringUtils.isEmpty(content)) {
                    XmbUtils.showMessage(PostActivity.this,"请输入内容，好内容才能上头条哦~");
                    return;
                } else {
                    //设置为不可点击，防止重复提交
    ...
    private class ModifyAsyncTask extends WeakAsyncTask<Void, Void, String, PostActivity> {

        public ModifyAsyncTask(PostActivity mUpRecordFragment) {
            super(mUpRecordFragment);
        }

        @Override
        protected void onPreExecute(PostActivity upRecordFragment) {
            super.onPreExecute(upRecordFragment);
            showWaitDialog();
        }

        @Override
        protected String doInBackground(PostActivity mUpRecordFragment,
                                        Void... params) {
            List<File> fileList = new ArrayList<>();
            for (int i = 0; i < pictures.size(); i++) {
                Uri uri = Uri.parse(pictures.get(i).getOriginalUri());
                String img_path = FileUtils.getPath(PostActivity.this, uri);
                Bitmap bmp = BitmapUtils.getCompressedImage(img_path, 2);
                try {
                    String f = BitmapUtils.saveMyBitmap(bmp, "tmp" + i, PostActivity.this);
            ...
            try {
                pm.put("post_title",title_edit.getText().toString());
                pm.put("post_content",content_edit.getText().toString());
                pm.put("circle_id",circle_id+"");
                pm.put("session[uid]", AppContext.getLoginUid() + "");
                pm.put("session[sid]", ApiHelper.mSessionID);
                String response = HttpUtility.post("http://api.xiaomabao.com/UserCircle/add_post", null, pm, "photo", fileList);
                return response;
            } catch (Exception e) {
        ...
        @Override
        protected void onPostExecute(PostActivity mUpRecordFragment, String result) {
            hideWaitDialog();
            try {
                JSONObject jsonObject = new JSONObject(result);
                if (jsonObject.getInt("status") != 1) {
                    toolbar_right_text.setEnabled(true);
                    XmbUtils.showMessage(PostActivity.this, jsonObject.getString("info"));
                    return;
                }
                XmbUtils.showMessageRefreshCircle(PostActivity.this, jsonObject.getString("info"));
            } catch (JSONException e) {
                XmbUtils.showMessage(PostActivity.this, "网络异常，未知错误");
            }
        }
    }
}
